...
def stream_chat(model, messages):
    try:
        ...
        logging.info(f"Model: {model}, Message: {messages}, Response: {response}")
        return response
    ...
    if prompt := st.chat_input("질문해주세요."):
        ...
        with st.spinner("응답 생성하는 중.."):
            try:
                ...
                response_messages_with_duration = f"{response_message}\n\nDuration: {duration:.2f} seconds"
                st.session_state.messages.append({"role":"assistant", "content": response_messages_with_duration})
